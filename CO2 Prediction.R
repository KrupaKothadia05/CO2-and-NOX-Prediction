...
# Train- Test Split
dataset <- dataframe%>%
  dplyr::select(-3,-7,-11,1,2,4,5,6,8,9,10)
head(dataset)
...
# Fitting model
regressor = lm(formula = CO ~ AT + AFDP + GTEP + TIT + TEY + CDP, data = training_set)
...
# Lasso Regression
train_x <- model.matrix(CO ~ ., training_set)[, -c(8)]
train_y <- log(training_set$CO)

test_x <- model.matrix(CO ~ ., test_set)[, -c(8)]
test_y <- log(test_set$CO)
